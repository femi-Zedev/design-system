        ...
        <>
          <div key={index} className='flex items-center gap-2'>
            ...
            <span className={cn('h-3 w-3 rounded-full',
              currentStep == index + 1 && 'bg-brand-500',
              currentStep > index + 1 && 'bg-brand-500',
              currentStep < index + 1 && 'border-2 border-slate-400',
            )
            } />

            <p className={cn(`font-medium text-sm max-md:w-max`,
              currentStep == index + 1 ? 'text-brand-500' : 'text-slate-500',
            )}>
          ...
          {index !== steps.length - 1 && (
            <span className={
              cn('min-w-8 h-0.5 rounded-xxs', currentStep >= index + 1 ? 'bg-brand-500' : 'bg-slate-200')
            }></span>
          )}
        </>
      ))}
